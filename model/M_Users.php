...
//
	...
	// результат	- true или false
	//
	public function Login($phoneNumber, $password, $remember = true)
	{
		// вытаскиваем пользователя из БД 
		$user = $this->GetByLogin($phoneNumber);
	...
	//
	public function GetByLogin($phoneNumber)
	{	
		$t = "SELECT * FROM users WHERE phone_number = '%s'";
		$query = sprintf($t, mysql_real_escape_string($phoneNumber));
		$result = $this->msql->Select($query);
		return $result[0];
	}
	...
	// Получает пользователя по логину
	//
	public function GetByIdVk($idVk)
	{	
		$t = "SELECT * FROM users WHERE id_vk = '%s'";
		$query = sprintf($t, mysql_real_escape_string($idVk));
		$result = $this->msql->Select($query);
		return $result[0];
	}
	...
	private function GetSid()
	{
		...
		if ($sid == null && isset($_COOKIE['phoneNumber']))
		{
			$user = $this->GetByLogin($_COOKIE['phoneNumber']);
	...
	public function regVkUser($object){
